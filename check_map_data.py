...
print('各城市的地图数据统计:')
print('='*100)
print(f'{"城市":<35} {"总活动数":>10} {"有地图数据":>12} {"无地图数据":>12} {"地图覆盖率":>12}')
print('-'*100)

for location, total, with_poly, without_poly in results:
    coverage = f"{(with_poly/total*100):.1f}%" if total > 0 else "0%"
    print(f'{location:<35} {total:>10} {with_poly:>12} {without_poly:>12} {coverage:>12}')
...
for (city,) in cities:
    ...
    if result:
        ...
        polyline_preview = polyline[:50] + "..." if polyline and len(polyline) > 50 else (polyline if polyline else "")
