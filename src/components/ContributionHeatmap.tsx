...
export function ContributionHeatmap({
  ...
  const presentDisplayTypes = useMemo(() => {
    ...
    return (['Run', 'Walk', 'Ride', 'Hike', 'Training'] as const).filter((t) =>
      types.has(t)
    );
  ...
  const gymMonthlyData = useMemo(() => {
    ...
    return Array.from({ length: 12 }, (_, m) => {
      ...
      const byType = Object.fromEntries(
        WORKOUT_TYPES.map((t) => [
          t,
          monthActs.filter((a) => a.type === t).length,
        ])
      );
  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-5">
      <style>{`
                ...
                >
                  ...
                  >
                    {m.total > 0 && (
                      ...
                      >
                        {WORKOUT_TYPES.filter((t) => m.byType[t] > 0).map(
                          (t) => {
